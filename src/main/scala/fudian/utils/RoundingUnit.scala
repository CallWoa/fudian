...
object RoundingUnit {
  def apply(in: UInt, rm: UInt, sign: Bool, width: Int, sticky_in: Option[Bool] = None): RoundingUnit = {
    ...
    val sticky = if(sticky_in.isDefined) sticky_in.get else in_pad.tail(width + 1).orR
