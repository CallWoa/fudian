...
int main(int argc, char* argv[]) {
    ...
    while(scanf("%lx", &a) != EOF){
        ...
        scanf("%lx %lx", &ref_result, &ref_fflags);
        module.clock = 0;
        module.io_in_bits_src_0 = a;
        if (op == 0) module.io_in_bits_src_1 = b;
        module.io_isSqrt = op;
        module.io_in_bits_rm = rm;
        module.io_in_valid = 1;
        module.io_out_ready = 1;
        module.eval();
        contextp->timeInc(1);
        tfp->dump(contextp->time());
        ...
        dut_result = module.io_out_bits_result;
        dut_fflags = module.io_out_bits_fflags;
        if( (dut_result != ref_result || dut_fflags != ref_fflags) ){
            if (op == 0) printf("[%ld] input: %lx %lx\n", cnt, a, b);
                else printf("[%ld] input: %lx\n", cnt, a);
            printf("[%ld] dut_sum: %lx dut_fflags: %lx\n", cnt, dut_result, dut_fflags);
